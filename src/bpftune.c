...
void do_help(void)
{
	fprintf(stderr,
		...
		"		     { -d|--debug} {-D|--daemon}\n"
		"		     { -c|--cgroup cgroup_path}\n"
		"		     { -L|--legacy}\n"
		"		     { -h|--help}}\n"
		"		     { -l|--libdir library_path}\n"
		"		     { -p|--port port}\n"
		"		     { -q|--query query}\n"
...
int main(int argc, char *argv[])
{
	...
	if (err) {
		if (library_dir) {
			bpftune_log(LOG_DEBUG, "could not initialize tuners in '%s': %s (will try custom library_dir '%s')\n",
				    BPFTUNER_LIB_DIR, strerror(-err), library_dir);
		} else {
